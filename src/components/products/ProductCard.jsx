...
const ProductCard = ({ data, version = 1 }) => {
  const { likedProducts, toggleLike } = useLikedStore();
  return (
    <Link
      href={
        version === 2
          ? `/browse/${data?.pineconeMetadata?.productHandle}`
          : `/products/browse/${data?.pineconeMetadata?.productHandle}`
      }
      className=" w-fit"
    >
      <div className=" relative w-[377px] border p-3 h-[377px] border-[#C6C2EC] rounded-[30px]">
        <div className=" border bg-white border-[#E1E1E1] w-full h-full rounded-[24px] flex flex-col items-center justify-center p-4">
          <img
