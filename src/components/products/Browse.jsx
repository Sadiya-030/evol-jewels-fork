...
const Browse = ({ data, isLoading, version, engagement = false }) => {
  ...
  const filteredGrouped = engagement ? { rings: grouped.rings || [] } : grouped;
  const groupKeys = Object.keys(filteredGrouped);
  ...
  return (
    <div ref={browseRef} className=" h-[1620px] overflow-y-scroll  ">
      ...
      {isLoading ? (
        ...
        <div className="mt-[65px]  pb-5 ">
          <p className="capitalize font-ethereal px-[100px] mb-[30px] text-[#302B2C] text-[38px]">
            {groupKeys[0]}
          </p>
        ...
        <div className=" pb-[20px] overflow-scroll">
          {groupKeys?.map((type) => (
              ...
              <div className="flex no-scrollbar gap-8 overflow-x-scroll cursor-grab touch-pan-x" style={{ WebkitOverflowScrolling: "touch", scrollBehavior: "smooth" }}>
                {filteredGrouped[type]?.map((item, i) => (
                  <div
                    key={i}
                    className={`${i === 0 && "ml-[100px]"} ${
                      i === filteredGrouped[type].length - 1 && "mr-[100px]"
                    }`}
                  >
                    <ProductCard version={version} data={item} />
                  </div>
