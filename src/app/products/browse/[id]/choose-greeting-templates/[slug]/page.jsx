...
const page = () => {
  ...
  const handleOptionClick = async (option) => {
    ...
    try {
      const res = await fetch(
        "https://evol.thumbstack.dev/api/products/greet-message",
        {
          ...
          body: JSON.stringify({
            pick: option,
            tone: "Keep Original",
            template: selectedCard.templateName, // use current template
            words: wordLimit,
      ...
      if (data.success && data.data?.greetingMessage) {
        setSelectedCard({ ...selectedCard, msg: data.data.greetingMessage });
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (selectedCard?.msg) {
      setgreetingDataText(selectedCard.msg); // hydrate textarea
    }
  }, [selectedCard.msg]);
      ...
      <div className="min-h-screen h-full w-full  relative z-50 bg-white/70">
          ...
          <div className="overflow-auto no-scrollbar flex items-center gap-[24px]">
            {steps === 1
              ? generateMsgPicks.map((item, index) => {
                  const isSelected = grtpicks === item;
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: isSelected ? "#DBC3EF" : "#7C50E312",
                      }}
        ...
        <div className=" px-[100px] flex flex-col  h-fit  gap-6 items-center">
          ...
          <p
            ...
            className="px-10 py-4  font-ethereal flex items-center justify-center  text-[40px]  h-[103px] w-full rounded-[2000px] bg-[#F4EFEFCC] text-[#302B2C] font-light"
          >
            Write my message
          </p>
        </div>
